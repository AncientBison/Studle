...
import { Box, Button, TextField, ThemeProvider, createTheme, Dialog, Typography, CircularProgress, DialogContent, DialogActions, DialogContentText, DialogTitle } from '@mui/material';
...
function CreateTopicsPage(properties) {
  const { mainTheme, setData, setSetData, updateNextStepButtonEnabled, nextId, addWarning, removeWarning, editingSetName, changePage } = properties;
    ...
    <Box display='flex' alignItems='center' flexDirection='column' height="85%" marginTop="5%">
      ...
      {(editingSetName ? setData.topics.length >= 1 : true) && (
        <Box display='flex' alignItems='center' flexDirection='column' height="100%" width="100%">
          <DeleteSetDialog mainTheme={mainTheme} open={deleteSetDialogOpen} setOpen={setDeleteSetDialogOpen} setName={setData.name} changePage={changePage} />
          <SetNameBox
            mainTheme={mainTheme}
          ...
          <TopicContainer mainTheme={mainTheme} nextId={nextId} updateNextStepButtonEnabled={updateNextStepButtonEnabled} setData={setData} setSetData={setSetData} addWarning={addWarning} removeWarning={removeWarning} />
        </Box>
      )}
    </Box>
  );
...
function TopicContainer(properties) {
  const { mainTheme, nextId, updateNextStepButtonEnabled, addWarning, removeWarning, setData, setSetData } = properties;
  const [pathsInterfering, setPathsInterfering] = useState([]);

  const topicExists = (name, path) => {
    return setData.topics.some((topic) => topic.path === path && topic.name === name);
  }

  const topicWithDifferentNameExists = (name, path) => {
    const targetPath = getPathWithDifferentName(name, path);
    return setData.topics.some((topic) => topic.path === targetPath && topic.name === name);
  }
    ...
    if (!topicWithDifferentNameExists(newName, path)) {
      let newTopics = setData.topics.map((topic) => {
        if (topic.path === path && topic.name === currentName) {
          return { ...topic, name: newName, path: getPathWithDifferentName(newName, path) };
        } else {
          return topic;
        }
      });
  ...
  const changePathName = (newName, path, topicsPassthrough) => {
    ...
    for (const factId of Object.keys(newFacts)) {
      ...
      if (factId.startsWith(path + factSplitSeparator)) {
        ...
        newFacts[factId.replace(path, newPath, 1)] = { ...factData, path: factData.path.replace(path, newPath, 1) }
      }
    }
  ...
  const deleteTopic = (name, path) => {
    if (topicExists(name, path)) {
      ...
      for (const topic of setData.topics) {
        if (topic.path !== path && !topic.path.startsWith(path + pathSplitSeparator)) {
          continue;
        }
        newTopics.splice(newTopics.findIndex(deletingTopic => deletingTopic.path === topic.path), 1);
      };
  ...
  const setTopicId = (name, path, id) => {
    let newTopics = setData.topics.map((topic) => {
      if (topic.path === path && topic.name === name) {
        return { ...topic, id: id };
      } else {
        return topic;
      }
    });
    updateTopicsAndFacts(newTopics);
  }
  ...
  const topicSort = (topics, a, b) => {
    ...
    for (let i = 0; i < Math.min(aParts.length, bParts.length); i++) {
      ...
      if (aPart !== bPart) {
        ...
        const aTopic = topics.find(obj => obj.path === aParts.slice(0, i + 1).join(pathSplitSeparator));
        const bTopic = topics.find(obj => obj.path === bParts.slice(0, i + 1).join(pathSplitSeparator));
      ...
      {setData.topics.map((topic) => {
        ...
        return (
          <TopicBox
            ...
            actionsDisabled={pathsInterfering.length >= 1 ? !pathsInterfering.includes(topic.path) : false}
...
function DeleteSetDialog(properties) {
  ...
  return (
    <Dialog open={open}>
      <Box display="flex" sx={{ justifyContent: "center", alignItems: "center", width: "100%", height: "100%" }}>
        <Box display="flex" flexDirection="column" sx={{
          ...
          <DialogTitle cursor="pointer" fontSize="3em" sx={{ ...disableTextSelection, textAlign: "center" }}>
            Delete Set
          </DialogTitle>
          <DialogContentText marginTop="1.5em" cursor="pointer" fontSize="2em" sx={{ ...disableTextSelection, textAlign: "center", width: "85%" }}>
            {`Deleting this set is permanent. Type the name of the set (${setName}) to confirm.`}
          </DialogContentText>
          <TextField value={typedText} error={typedText !== setName} autoFocus
            onChange={(event) => {
            ...
            inputProps={{ sx: { fontSize: "2em" }, autoCapitalize: 'off', autoComplete: "one-time-code", autoCorrect: "false", spellCheck: "false" }}
            sx={{
              width: "calc(100% - 2em)",
              margin: "1em",
              marginTop: "2em"
          ...
          <DialogActions display="flex" sx={{width: "100%", paddingLeft: 0, paddingRight: 0}}>
            <Box display="flex" sx={{justifyContent: "space-around", width: "100%"}}>
              ...
              <Button variant="contained" disabled={typedText !== setName}
                sx={{
                  textTransform: "none",
                  width: "max(50%, min-content)",
                  height: "15%",
...
function SetNameBox(properties) {
  const { mainTheme, setSetName, addWarning, removeWarning, takenSetNames, startingText, editingSetName, setDeleteSetDialogOpen } = properties;
    ...
    <ThemeProvider theme={robotoTheme}>
      <Box sx={{
        ...
        <TextField
          ...
          inputProps={{
            maxLength: 50,
            style: { fontSize: '7vh', textAlign: 'center', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', }
          }}
          variant="standard">
        </TextField>
        {editingSetName && (
...
function TopicBox(properties) {
  const { mainTheme, startingText, addTopic, path, renameTopic, deleteTopic, getPathWithDifferentName, topicWithDifferentNameExists, addWarning, removeWarning, indent, setPathsInterfering, actionsDisabled, addSubtopicDisabled } = properties;
  const [warningText, setWarningText] = useState("");
  const [topicName, setTopicName] = useState(startingText);
  const [text, setText] = useState(startingText === "extraTopicBox" ? "" : startingText ?? "");
  ...
  const attemptChangeToNewName = () => {
    ...
    if (topicWithDifferentNameExists(newName, path) && getPathWithDifferentName(newName, path) !== path) {
      ...
      <Box sx={{
        ...
        <TextField
          ...
          sx={{
            ...
            "& .MuiInputBase-root": {
              height: "100%",
              fontSize: {xl: "3.5rem", lg: "3.5rem", md: "3rem", sm: "2.5rem", xs: "2rem"},
            },
            "& .MuiFormHelperText-root": {
              fontSize: "2vh",
              marginLeft: "1em",
        ...
        {topicName != "extraTopicBox" && (
          <Button variant="outlined" size="large"
            ...
            sx={{
              ...
              fontWeight: 'bold',
              width: "7em",
              fontSize: {xl: "2em", lg: "2em", md: "2em", sm: "1.5", xs: "1em"},
              '&:hover': {
            ...
            onClick={() => addTopic("extraTopicBox", path + pathSplitSeparator + sanitize("extraTopicBox"))}
          >
            Add Subtopic
          </Button>
        )}
