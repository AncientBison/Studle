...
function StudySetOptions(properties) {
  const { mainTheme, studyMode, setStudyMode, topics, togglePath, pathIsEnabled, useLastSetting } = properties;

  return (
    <Box display='flex' alignItems='center' flexDirection='column' height="85%" marginTop="5%">
      <SetModeSelector mainTheme={mainTheme} studyMode={studyMode} setStudyMode={setStudyMode} />
      {topics.map(topic => (
        <StaticTopicBox
          mainTheme={mainTheme}
...
function SetModeSelector(properties) {
  ...
  return (
    ...
    >
      <ToggleButton value="multiple" sx={{
        textTransform: "none",
        fontSize: "2em",
...
function StaticTopicBox(properties) {
  const { mainTheme, topicName, indent, path, togglePath, pathIsEnabled, useLastSetting } = properties;
  const [toggled, setToggled] = useState(useLastSetting ? pathIsEnabled(path) : true);
  ...
  return (
    <ThemeProvider theme={robotoTheme}>
      <Box sx={{
        ...
        <Typography sx={{ ...disableTextSelection, fontSize: { xl: "4.5em", lg: "4em", md: "4em", sm: "3em", xs: "3em" }, width: "100%", textAlign: "center", textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} >
          {topicName}
        </Typography>
        <PathToggleButton mainTheme={mainTheme} path={path} togglePath={togglePath} toggled={toggled} setToggled={setToggled}></PathToggleButton>
      </Box>
    </ThemeProvider >
  );
}
...
function PathToggleButton(properties) {
  ...
  return (
    <Button
      ...
      sx={{
        backgroundColor: (toggled) ? mainTheme.palette.buttonSuccess.main : mainTheme.palette.buttonSuccess.disabled,
        width: "3em",
        height: "100%",
        '&:hover': {
          backgroundColor: (toggled) ? mainTheme.palette.buttonSuccess.hover : mainTheme.palette.buttonSuccess.hoverDisabled
        },
      }}>
      <KeyboardDoubleArrowRightIcon sx={{ color: mainTheme.palette.contrast.main, transition: "all 300ms", transform: (toggled ? "scale(2)" : "scale(2) rotate(-180deg)") }} />
    </Button>
  )
}
