...
function StudySetPage(properties) {
  const { mainTheme, changePage, studyingSetName, setStudySetPageInfoPage, setStudyingSetName, email, settingEnabled, studyingShared, sharedSetData } = properties;
    ...
    if (!setData) {
      if (!studyingSetName) {
        ...
        if (!studyingShared.current) {
          ...
          setEnabledPaths(sharedSetData.current.topics.map(topic => topic.path));  
        }
      } 
    }
  }, []);
    ...
    <Box sx={{ paddingBottom: "5%" }}>
      <AppBar position='sticky' color="primary" sx={{ height: "3.75em" }}>
        <Box sx={{
          ...
          <Typography marginLeft="0.5em" cursor="pointer" sx={{ ...disableTextSelection, cursor: "pointer", fontSize: { xl: "2.2em", lg: "2.2em", md: "2.2em", sm: "2em", xs: "1.5em" } }} onClick={() => changePage("home")}>
            Studle
          </Typography>
          <Typography style={{ marginRight: "1.5vh" }} align='center' sx={{...disableTextSelection, fontSize: { xl: "2.2em", lg: "2.2em", md: "2.2em", sm: "1.9em", xs: "1.5em" }}} >
            {studyingSetName}
          </Typography>
        </Box>
      </AppBar>
        ...
        {step == 2 && (
          <Button
            ...
            sx={{
              ...
              fontWeight: 'bold',
              height: "calc(100% - 1em)",
              fontSize: { xl: "2.5em", lg: "2em", md: "2em", sm: "1.7em", xs: "1.5em" },
              '&:hover': {
      ...
      {setData && step == 0 && (
        <StudySetOptions mainTheme={mainTheme} studyMode={studyMode} setStudyMode={setStudyMode} useLastSetting={enabledPaths.length >= 1} topics={setData.topics} togglePath={togglePath} pathIsEnabled={pathIsEnabled} />
      )}
      {setData && step == 1 && (
        <StudySetQuestioner mainTheme={mainTheme} studyMode={studyMode} enabledPaths={enabledPaths} setData={setData} setStep={setStep} answeredQuestions={answeredQuestions} setAnsweredQuestions={setAnsweredQuestions} />
      )}
      {setData && step == 2 && (
        <StudySetSummary mainTheme={mainTheme} studyingShared={studyingShared} topics={setData.topics} setName={studyingSetName} pathIsEnabled={pathIsEnabled} answeredQuestions={answeredQuestions} enabledPaths={enabledPaths} settingEnabled={settingEnabled} />
      )}
    </Box>
  );
}
