import { Box, Modal, Typography, TextField, ThemeProvider, createTheme, Button, Dialog, DialogContentText, DialogContent, DialogTitle, DialogActions } from '@mui/material';
...
import { desanitize } from '../sanitizer';
...
function StudySetSummary(properties) {
  const { mainTheme, studyingShared, topics, setName, pathIsEnabled, answeredQuestions, enabledPaths, settingEnabled } = properties;
  ...
  const getAccuracyByPath = (path) => {
    ...
    for (const question of answeredQuestions) {
      if (question.factPath === path || question.factPath.startsWith(path + pathSplitSeparator)) {
        questionCount += 1;
        if (question.correct) {
          questionsCorrect += 1;
        }
  ...
  const getIncorrectQuestionsByPath = (path) => {
    ...
    const incorrectQuestions = questionsInPath.filter(question => !question.correct);
    const incorrectQuestionsFormatted = incorrectQuestions.map(question => ({ question: question.question, correctAnswer: question.correctAnswer, answer: question.answer }));
  ...
  const addEarnedTickets = async (questionsCorrect) => {
    if (questionsCorrect > 0) {
      ...
      try {
        data = await fetch(BACKEND_URL + "/addTickets/" + questionsCorrect + "/" + encodeURIComponent(localStorage.getItem("encryptedEmail")), {
          method: "GET",
          credentials: "include",
          mode: 'cors',
          headers: {
      ...
      if (json.type == "error") {
        setTicketDialogText("There was an error while trying to set your tickets. We're sorry for the inconvenience. If the issue persists, please use the feedback form in the hamburger menu.");
      } else {
        let text = "";
        if (json.data.overflow < questionsCorrect) {
          text = "You earned " + questionsCorrect + (questionsCorrect !== 1 ? " tickets." : " ticket.");
        ...
        if (json.data.overflow > 0) {
          ...
          text += ("You have reached the maximum number of tickets you can earn today. You can earn more tomorrow!");
        }
  ...
  useEffect(() => {
    ...
    const saveAnalytics = async () => {
      setOverallSetAnalytics(await saveSetAccuraciesAndGetChartData(setName, setAccuracies.current));
    }
  ...
  return (
    <ThemeProvider theme={robotoTheme} >
      <TicketDialog mainTheme={mainTheme} open={ticketDialogOpen} setOpen={setTicketDialogOpen} text={ticketDialogText} />
      {overallScore == 100 && settingEnabled("show_confetti") && (
        <Confetti />
      )}
      <Typography marginBottom="0.7em" marginTop="1em" sx={{ ...disableTextSelection, fontSize: { xl: "3em", lg: "3em", md: "3em", sm: "2em", xs: "1.5em" }, textAlign: 'center' }}>
        {summaryText}
      </Typography>
      <Box display="flex" sx={{ flexDirection: "column", justifyContent: 'center' }}>
        <Typography sx={{ ...disableTextSelection, fontSize: "3em", textAlign: 'center' }}>
          {overallScore != null ? `${overallScore}%` : "No Questions Answered"}
        </Typography>
        <Box display="flex" sx={{ justifyContent: "center" }}>
          {overallScore != null && overallScore !== -1 && (
            <LinearProgress variant="determinate" value={overallScore > 0 ? overallScore : 100}
              sx={{
                width: "80%",
                height: "3vh",
                borderRadius: "5px",
      ...
      <Box display='flex' alignItems='center' flexDirection='column' height="85%" marginTop="2%">
        {topics.filter(topic => pathIsEnabled(topic.path)).map(topic => {
        ...
        {!studyingShared.current && (
          (overallSetAnalytics ?? []).length >= 2 ? ( 
            <Box sx={{marginTop: "2rem", width: "100%", height: "max-content"}}>
              <SetAnalyticsGraph data={overallSetAnalytics} topics={topics} />
            </Box>
          ) : 
            <Typography sx={{ ...disableTextSelection, marginTop: "2rem", fontSize: "1.5rem", textAlign: 'center' }}>Study this set again to view analytics</Typography>
        )}
      </Box>
    </ThemeProvider>
  );
...
const SetAnalyticsTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
        <Box sx={{width: "10rem", minHeight: "6rem", backgroundColor: "#fff", borderRadius: "4px", border: "1px solid #cdcdcd"}}>
          ...
          {Object.entries(payload[0].payload).map(([key, value]) => {
            ...
            if (key === "overallScore") {
              return (
                <Typography sx={{textAlign: "left", marginLeft: "0.5rem", fontSize: "0.8rem", fontWeight: "bold", color: getDistinctHashedColor("overallScore")}} key={key}>
                  Overall Score: {value}%
                </Typography>
              )
            }
            return (
              <Typography sx={{textAlign: "left", marginLeft: "0.5rem", fontSize: "0.8rem", fontWeight: "bold", color: getDistinctHashedColor(key)}} key={key}>
  ...
  return (
    <ResponsiveContainer width="100%" aspect={3} >
      ...
      >
        ...
        <XAxis dataKey={pathSplitSeparator + "date"} tickFormatter={formatDate} />
        ...
        />
        <Line type="monotone" name={"Overall Score"} dataKey={"overallScore"} stroke={getDistinctHashedColor("overallScore")} activeDot={{ r: 8 }} />
        {topics.map((topic, index) => {
          ...
          return (
            <Line type="monotone" name={desanitize(topic.path)} stroke={getDistinctHashedColor(topic.path)} dataKey={topic.path} activeDot={{ r: 8 }} key={index} />
          );
        })}
      </LineChart>
    </ResponsiveContainer>
...
function TicketDialog(properties) {
  ...
  return (
    <Dialog open={open}>
      <DialogTitle sx={{ fontWeight: "bold" }}>Adding your tickets...</DialogTitle>
      <DialogContent>
        <DialogContentText sx={{ color: mainTheme.palette.contrast.main, fontWeight: "bold" }}>
          {text}
        </DialogContentText>
      </DialogContent>
      <DialogActions>
...
function StaticTopicBox(properties) {
  const { mainTheme, topicName, path, indent, accuracy, incorrectQuestions, addAccuracyToSetAccuracies } = properties;
  ...
  return (
    <ThemeProvider theme={robotoTheme}>
      <Box sx={{
        ...
        <Typography sx={{ ...disableTextSelection, fontSize: { xl: "4.5em", lg: "4em", md: "3em", sm: "2em", xs: "2em" }, width: "100%", textAlign: "center", overflow: 'hidden', whiteSpace: 'pre-wrap' }} >
          {topicName}
        </Typography>
        <Box margin="0.5em" position="relative" sx={{ alignItems: "center", justifyContent: "center" }}>
      ...
      {incorrectQuestions.map(question => (
        <IncorrectQuestionBox mainTheme={mainTheme} question={question} indent={indent + 1} key={question.question} />
      ))}
    </ThemeProvider >
  );
}
...
function IncorrectQuestionBox(properties) {
  ...
  const questionWords = question.question.split(/(<BLANK>|\s+)/).filter(Boolean).filter(word => word !== "  ")
  const questionButtons = [
    ...(questionWords.map((word, index) =>
      <Typography key={index * 3} sx={{ ...disableTextSelection, fontSize: { xl: "1.6em", lg: "1.3em", md: "1em", sm: "0.8em", xs: "0.8em" }, marginLeft: "0.2em", fontWeight: "bold", borderBottom: (word.includes("<BLANK>") ? `0.2em solid ${mainTheme.palette.mistake.error}` : `0.2em solid ${mainTheme.palette.mistake.main}`) }}>
        {word === "<BLANK>" ? question.answer : desanitize(word)}
      </Typography>
    )),
    <Typography key={-3} sx={{ ...disableTextSelection, fontSize: { xl: "1.6em", lg: "1.3em", md: "1em", sm: "0.8em", xs: "0.8em" }, marginLeft: "0.2em", fontWeight: "bold" }}>
      (Correct Answer:
    </Typography>,
    <Typography key={-2} sx={{ ...disableTextSelection, fontSize: { xl: "1.6em", lg: "1.3em", md: "1em", sm: "0.8em", xs: "0.8em" }, marginLeft: "0.2em", fontWeight: "bold", borderBottom: `0.2em solid ${mainTheme.palette.mistake.correct}` }}>
      {question.correctAnswer}
    </Typography>,
    <Typography key={-1} sx={{ ...disableTextSelection, fontSize: { xl: "1.6em", lg: "1.3em", md: "1em", sm: "0.8em", xs: "0.8em" }, fontWeight: "bold" }}>
      )
    </Typography>,
  ]
  ...
  return (
    <ThemeProvider theme={robotoTheme}>
      <Box sx={{
        ...
        <CloseIcon sx={{ color: mainTheme.palette.mistake.error, width: "2em", height: "2em" }} />
        {questionButtons}
      </Box>
    </ThemeProvider >
  );
...
async function saveSetAccuraciesAndGetChartData(setName, setAccuracies) {
  ...
  const accuraciesWithDate = { ...notNullAccuracies, [pathSplitSeparator + "date"]: Date.now() };
