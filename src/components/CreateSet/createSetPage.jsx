...
const CreateSetPage = memo(function CreateSetPageComp(properties) {
  const {
    mainTheme,
    setCreateSetPageInfoPage,
    changePage,
    editingSetName,
    email,
  } = properties;
  ...
  const [nextStepButtonEnabled, setNextStepButtonEnabled] = useState(
    editingSetName ? true : false
  );
  ...
  const updateNextStepButtonEnabled = (topics) => {
    ...
    const extraTopicBoxIndex = newTopics.findIndex(
      (topic) =>
        topic.name == 'extraTopicBox' &&
        !topic.path.includes(pathSplitSeparator)
    );
    ...
    if (editingSetName) {
      fetch(BACKEND_URL + '/editStudle', {
        ...
        body: JSON.stringify({
          ...
          data: JSON.stringify(
            getCleanSetData(getSetDataWithoutUnusedAnswserTypes(setData))
          ),
        }),
      }).then(async () => {
        changePage('home');
...
export { CreateSetPage, factSplitSeparator,pathSplitSeparator };
