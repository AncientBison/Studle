...
function CreateFactsPage(properties) {
  const { mainTheme, setData, setSetData, addWarning, removeWarning } =
    properties;
  ...
  const addFactToTopicPath = (factData) => {
    ...
    setSetData((setData) => {
      ...
      if (!Object.keys(setData.facts).includes(factId)) {
        newSetData.facts[factId] = {
          fact: factData.fact,
          blanks: factData.blanks,
          path: path,
        };
...
function FactBox(properties) {
  const { mainTheme, factData, path, indent, removeFactFromTopicPath } =
    properties;
  ...
  const updateBlanks = () => {
    ...
    for (let i = 0; i < factData.fact.length; i++) {
      ...
      if (lastCharWasBlank != isBlank || factData.fact[i - 1] === ' ') {
        ...
        const color = lastCharWasBlank
          ? mainTheme.palette.primary.main
          : mainTheme.palette.contrast.main;
        newBlankBoxes.push(
          <Typography
            style={{
              fontSize: '2vh',
              fontWeight: 'bold',
              textDecoration: lastCharWasBlank
...
function AddFactModal(properties) {
  ...
  const selectionOverlapsOtherBlanks = (selection) => {
    for (const blank of Object.values(blanks)) {
      if (
        selection.startIndex <= blank.endIndex &&
        selection.endIndex >= blank.startIndex
      ) {
        return true;
      }
    }
      ...
      >
        ...
        >
          ...
          <InfoDialog
            ...
            mainTheme={mainTheme}
            title={step === 0 ? 'Create Fact' : 'Select Key Information'}
            content={
              <Box
                width="100%"
                display="flex"
          ...
          >
            {step === 0
              ? 'Facts need to have three to four significant words, such as names, dates, places, or verbs'
              : 'Select three to four significant words, such as names, dates, places, or verbs'}
          </Typography>
          {step == 0 && (
            <TextField
              ...
              onChange={(event) => {
                ...
                if (
                  factIdExists(
                    path + factSplitSeparator + sanitize(event.target.value)
                  )
                ) {
                ...
                <Box sx={{ height: '100%' }}>
                  ...
                  <Box style={{ fontSize: '1rem' }}>
                    {Object.values(blanks).map((blank) => {
                      const blankString = fact.substring(
                        blank.startIndex,
                        blank.endIndex + 1
                      );
                      return (
                        ...
                        >
                          <Box width="100%" display="flex">
                            ...
                            <AnswerTypeSelector
                              mainTheme={mainTheme}
                              setBlankAnswerType={setBlankAnswerType}
                              blankStartIndex={blank.startIndex}
                              answerTypeItems={answerTypeItems}
                              addAnswerType={addAnswerType}
                            />
                            <Button
                              ...
                              sx={{
                                backgroundColor:
                                  mainTheme.palette.buttonDelete.main,
                                width: '3em',
                                '&:hover': {
                                  backgroundColor:
                                    mainTheme.palette.buttonDelete.hover,
                                },
                              }}
                            >
                              <DeleteIcon
          ...
          >
            ...
            <Button
              ...
              disabled={
                fact === '' ||
                !!warningText ||
                (step == 1 && Object.keys(blanks).length == 0) ||
  ...
  return (
    <Box sx={{ width: '45%', padding: '0.5em', textAlign: 'center' }}>
      <Typography style={{ fontSize: '0.75em' }}>
        Select or Add Answer Type
      </Typography>
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <Select
          value={answerType}
          onChange={(event) => handleAnswerTypeChange(event.target.value)}
          sx={{
            width: '100%',
          }}
          SelectDisplayProps={{
        ...
        >
          ...
          >
            <Box
              ...
              sx={{
                backgroundColor: mainTheme.palette.popup.main,
                width: 'max(25rem, fit-content)',
                alignItems: 'center',
                height: 'max(8rem, fit-content)',
              }}
            >
              <TextField
                value={newAnswerType}
                ...
                inputProps={{
                  maxLength: 20,
                  style: {
                    color: mainTheme.palette.popup.contraText,
                    padding: '0.3em',
                    textAlign: 'center',
                    textOverflow: 'ellipsis',
                    overflow: 'hidden',
