...
export function CreateExtraAnswersPage(properties) {
  const { mainTheme, setData, setSetData, addWarning, removeWarning } =
    properties;
  const [addExtraAnswersModalOpen, setAddExtraAnswersModalOpen] =
    useState(false);
  const [answerType, setAnswerType] = useState();

  const getAllAnswersForAnswerType = (answerType) => {
    const answers = setData.extraAnswers[answerType]
      ? setData.extraAnswers[answerType].map((extraAnswer) => ({
          answer: extraAnswer,
          editable: true,
        }))
      : [];
      ...
      blanks.forEach((blank) => {
        ...
        if (blankAnswerType === answerType) {
          ...
          if (
            !answers.find(
              (answer) =>
                answer.answer.toLowerCase() === answerString.toLowerCase()
            )
          ) {
  ...
  const answerTypeHasAnswer = (answerType, answer) => {
    return getAllAnswersForAnswerType(answerType).find(
  ...
  const addExtraAnswer = (answerType, extraAnswer) => {
    ...
    setSetData((setData) => {
      ...
      if (
        !allAnswersForAnswerType.find((answer) => answer.answer === extraAnswer)
      ) {
        if (!newSetData.extraAnswers[answerType]) {
  ...
  const checkAnswerTypesHaveEnoughAnswers = (answerTypes) => {
    ...
    for (const answerType of answerTypes) {
      const allAnswersForAnswerType = getAllAnswersForAnswerType(answerType);
      if (
        !(allAnswersForAnswerType.length >= minimumPossibilitiesPerAnswerTypes)
      ) {
...
function PossibilityBox(properties) {
  const { mainTheme, editable, answerTypeName, answer, removeExtraAnswer } =
    properties;
  ...
  return (
    <Box
      ...
      sx={{
        ...
        alignItems: 'center',
        border: '2px solid ' + mainTheme.palette.background.contrastText,
        marginBottom: '0.3em',
        padding: '0.15em',
        height: '3rem',
      }}
    >
      <Typography
        sx={{ lineHeight: 0, color: mainTheme.palette.background.contrastText }}
      >
        {answer}
      </Typography>
      {editable && (
