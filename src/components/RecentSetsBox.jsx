    ...
    >
      ...
      >
        ...
        >
          ...
          {fetchSetsStatus === '' && <CircularProgress />}
        </Box>
      </Box>
    </Box>
  );
...
function RecentSetBox(properties) {
    ...
    return (
      ...
      >
        <Box display="flex" alignItems="center" flexDirection="column">
          <DialogTitle id="alert-dialog-title">{'Share this set!'}</DialogTitle>
          <DialogContent>
            <DialogContentText
  ...
  const ShareButton = (properties) => {
    const { setShareDialogOpen } = properties;
    const [hovered, setHovered] = useState(false);
  ...
  return (
    ...
    >
      ...
      <Box
        display="flex"
        sx={{ width: smallScreen ? '50%' : '30%', overflow: 'auto' }}
      >
        <ShareButton
          setName={setName}
          setEditingSet={setEditingSet}
