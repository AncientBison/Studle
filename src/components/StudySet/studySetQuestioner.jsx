...
import { desanitize,sanitize } from '../../lib/sanitizer';
...
function StudySetQuestioner(properties) {
  ...
  const [writtenResponseFieldValue, setWrittenResponseFieldValue] =
    useState('');
    ...
    for (const factId of Object.keys(setData.facts)) {
      ...
      if (enabledPaths.includes(factData.path)) {
        for (const blank of Object.values(factData.blanks)) {
          newQuestions.push({
            question:
              sanitize(factData.fact.substring(0, blank.startIndex)) +
              '<BLANK>' +
              sanitize(factData.fact.substring(blank.endIndex + 1)),
            correctAnswer: factData.fact.substring(
              blank.startIndex,
              blank.endIndex + 1
            ),
  ...
  useEffect(() => {
    if (questions.length > 0) {
      if (questions[currentQuestionIndex]) {
        ...
        const newQuestionSplit = newQuestion.question
          .split('<BLANK>')
          .map((questionSegment) => desanitize(questionSegment));
  ...
  const submitWrittenAnswer = (answer) => {
    ...
    const answerCorrect =
      levenshteinDistance(answer, correctAnswer) / correctAnswer.length <
      writtenResponseQuestionInnaccuracy;
    setAnsweredQuestions((answeredQuestions) => [
      ...answeredQuestions,
      {
        ...questions[currentQuestionIndex],
  ...
  const getPossibleAnswerButtons = () => {
    ...
    if (
      !possibleAnswers.includes(questions[currentQuestionIndex].correctAnswer)
    ) {
    ...
    for (const possibleAnswer of possibleAnswers) {
      possibleAnswerButtons.push(
        <Button
          onClick={() => submitMultipleChoiceAnswer(possibleAnswer)}
          variant="contained"
          sx={{
            margin: '0.6em',
            width:
          ...
          key={
            currentQuestionIndex + pathSplitSeparator + sanitize(possibleAnswer)
          }
        >
          <Typography
            sx={{
          ...
          >
            {possibleAnswer[0].toUpperCase() +
              possibleAnswer.replace(possibleAnswer[0], '')}
          </Typography>
        </Button>
      );
    }
    ...
    <TextField
      ...
      sx={{
        width: displayTextWidth(
          answers,
          `${fontSize} ` + mainTheme.typography.fontFamily
        ),
      }}
      inputProps={{
        autoCapitalize: 'off',
        ...
        autoCorrect: 'false',
        spellCheck: 'false',
        sx: { fontSize: fontSize, padding: 0, margin: 0 },
      }}
    ></TextField>
  );
}
...
function getPossibleAnswers(setData, answerType, limit = 2147483647) {
  ...
  if (setData.extraAnswers[answerType]) {
    possibleAnswers = [
      ...getAnswerTypes(setData)[answerType],
      ...setData.extraAnswers[answerType],
    ];
  } else {
    possibleAnswers = getAnswerTypes(setData)[answerType];
  }
  return shuffleArray(
    removeDuplicates(limitArrayLength(possibleAnswers, limit))
  );
}
