...
import { CreateSetPage } from './components/CreateSet/createSetPage';
import { HomePage } from './components/homePage';
import { InfoDialog } from './components/infoDialog';
import { LandingPage } from './components/landingPage';
import { CLIENT_ID } from './components/loginManager';
import { MarketPage } from './components/Market/marketPage';
import { stickerInfo } from './components/Market/stickerInfo';
import getPageInfo from './components/pageInfo';
import { SettingsPage } from './components/Settings/settingsPage';
import { StudySetPage } from './components/StudySet/studySetPage';
...
function App() {
  ...
  useEffect(() => {
    for (const sticker of stickers) {
      if (
        savedStickers.current.find(
          (savedSticker) =>
            savedSticker.id === sticker.id &&
            (savedSticker.x !== sticker.x ||
              savedSticker.y !== sticker.y ||
              savedSticker.enabled !== sticker.enabled)
        )
      ) {
  ...
  const handleBuySticker = (id) => {
    setStickers([...stickers, { id, enabled: false, x: 0, y: 0 }]);
  };
  ...
  const pageInfo = getPageInfo(
    mainTheme,
    useMediaQuery(mainTheme.breakpoints.up('sm'))
  );
  ...
  return (
    <ThemeProvider theme={mainTheme}>
      ...
      {pageName && (
        <InfoDialog
          ...
          title={(
            ...
            .replace('Landing Page', 'Welcome!')
            .replace('CreateSet', camelCaseToTitleCase(createSetPageInfoPage))
            .replace('StudySet', camelCaseToTitleCase(studySetPageInfoPage))}
          content={
            pageName === 'createSet'
              ? pageInfo[pageName]
                ? pageInfo[pageName][createSetPageInfoPage]
                : ''
              : pageName === 'studySet'
              ? pageInfo[pageName]
                ? pageInfo[pageName][studySetPageInfoPage]
                : ''
              : pageInfo[pageName]
          }
        />
      ...
      <HamburgerMenu
        ...
        setShowStickerDialog={setShowStickerDialog}
      ...
      {pageName === 'createSet' && signedInEmail && (
        <CreateSetPage
          ...
          setCreateSetPageInfoPage={setCreateSetPageInfoPage}
        />
      )}
      {pageName === 'studySet' && signedInEmail && (
        <StudySetPage
          mainTheme={mainTheme}
          changePage={changePage}
          studyingSetName={studyingSetName}
          setStudySetPageInfoPage={setStudySetPageInfoPage}
          setStudyingSetName={setStudyingSetName}
          email={signedInEmail}
      ...
      >
        {stickers
          ? stickers
              .filter((sticker) => sticker.enabled && pageName === 'home')
              .map((sticker) => {
...
function HamburgerMenu(properties) {
  ...
  const actions = [
    ...
    },
    {
      ...
      name: 'Feedback',
      run: () => {
        window.open('https://forms.gle/9KExw4FGXMRUK8uG7', '_blank');
      },
    },
    {
    ...
    {
      icon: <AutoAwesomeMotionIcon sx={iconStyle} />,
      name: 'Stickers',
      run: () => {
    ...
    <Box>
      ...
      >
        {actions
          .filter((action) =>
            pageName !== 'landing'
              ? action.name === 'Stickers'
                ? pageName === 'home'
                : action.name.toLowerCase() !== pageName
              : action.name === 'Info'
          )
          .map((action) => (
            <SpeedDialAction
...
function DraggableSticker(properties) {
  const {  id, xPos, yPos, size, moveSticker } = properties;
  ...
  return (
    <Draggable
      ...
      bounds={'parent'}
      defaultPosition={{
        x: Math.ceil(xPos * window.innerWidth - imageSizePx / 2),
        y: Math.max(
          Math.min(
            Math.ceil(yPos * window.innerHeight - imageSizePx / 2),
            window.innerHeight - imageSizePx
          ),
          0
        ),
      }}
      onStop={(e, value) => {
        moveSticker(
          id,
          (value.x + imageSizePx / 2) / window.innerWidth,
          (value.y + imageSizePx / 2) / window.innerHeight
        );
      }}
    >
      <Box
      ...
      >
        <img
          aria-labelledby={id + '/handle'}
          src={stickerInfo[id].image}
          style={{
            ...disableInteraction,
            width: imageSizePx + 'px',
            height: Math.ceil(aspectRatio * imageSizePx),
